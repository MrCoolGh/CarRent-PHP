...
// Fetch logged-in admin's data for the navbar/sidebar
$adminData = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch ($action) {
        case 'get_users':
            $search = $_POST['search'] ?? '';
            $query = "SELECT id, first_name, last_name, email, phone, date_of_birth, user_type, created_at, profile_image FROM users WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ?";
            $searchTerm = "%{$search}%";
        ...
        case 'update_user':
            ...
            if (!empty($_POST['password'])) {
                $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
                $stmt = $conn->prepare("UPDATE users SET first_name=?, last_name=?, email=?, phone=?, date_of_birth=?, user_type=?, password=?, profile_image=? WHERE id=?");
                $stmt->bind_param("ssssssssi", $firstName, $lastName, $email, $phone, $dob, $userType, $password, $profileImagePath, $userId);
            } else {
    ...
    <div class="sidebar pe-4 pb-3">
        <nav class="navbar bg-light navbar-light">
            ...
            <div class="navbar-nav w-100">
                ...
                <a href="manageusers.php" class="nav-item nav-link active">
            ...
            <div class="bg-light text-center rounded p-4">
                ...
                <div class="table-responsive">
                    <table class="table text-start align-middle table-bordered table-hover mb-0" id="userTable">
                        ...
                        <tr><td colspan="10" class="text-center">Loading users...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                <form class="modal-content" id="addUserForm" onsubmit="saveNewUser(event)">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">Phone Number</label><input type="text" name="phone" class="form-control" required></div>
                        </div>
                        <div class="mb-3"><label class="form-label">Password</label><input type="password" name="password" class="form-control" placeholder="Enter user password" required></div>
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">User Type</label><select name="user_type" class="form-select" required><option value="customer">Customer</option><option value="manager">Manager</option><option value="admin">Admin</option></select></div>
                        </div>
                    </div>
        ...
        <div class="modal fade" id="editUserModal" tabindex="-1" aria-labelledby="editUserModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form class="modal-content" id="editUserForm" onsubmit="saveEditUser(event)">
                    <div class="modal-header"><h5 class="modal-title" id="editUserModalLabel">Edit User Information</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label">First Name</label><input type="text" id="editUserFirstName" name="first_name" class="form-control" required></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Last Name</label><input type="text" id="editUserLastName" name="last_name" class="form-control" required></div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label">Email</label><input type="email" id="editUserEmail" name="email" class="form-control" required></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Phone Number</label><input type="text" id="editUserPhone" name="phone" class="form-control" required></div>
                        </div>
                        <div class="mb-3"><label class="form-label">New Password</label><input type="password" name="password" class="form-control" placeholder="Leave blank to keep current password"></div>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label">Date of Birth</label><input type="date" id="editUserDOB" name="date_of_birth" class="form-control" required></div>
                            <div class="col-md-6 mb-3"><label class="form-label">User Type</label><select id="editUserType" name="user_type" class="form-select" required><option value="customer">Customer</option><option value="manager">Manager</option><option value="admin">Admin</option></select></div>
                        </div>
                    </div>
                    <div class="modal-footer">
...
<script>
    ...
    let deleteUserIdToProcess = null;
    ...
    async function fetchUsers(searchTerm = '') {
        ...
        try {
            const response = await fetch('manageusers.php', { method: 'POST', body: formData });
            ...
            tbody.innerHTML += `
                <tr id="userRow${u.id}">
                    ...
                    <td>
                        ...
                        <button class="btn btn-sm btn-danger" onclick="showDeleteConfirmModal(${u.id})">Delete</button>
                    </td>
                </tr>
            `;
        });
    ...
    function showDeleteConfirmModal(userId) {
        deleteUserIdToProcess = userId;
        var modal = new bootstrap.Modal(document.getElementById('confirmDeleteModal'));
        modal.show();
    }

    document.getElementById('deleteConfirmBtn').addEventListener('click', async function() {
        if (!deleteUserIdToProcess) return;
        const formData = new FormData();
        formData.append('action', 'delete_user');
        formData.append('id', deleteUserIdToProcess);
        ...
        try {
            ...
            if (result.success) {
                ...
                showGlobalMessage(result.message || 'Failed to delete user.', false);
            }
        } catch (error) {
            showGlobalMessage('An error occurred.', false);
        } finally {
            bootstrap.Modal.getInstance(document.getElementById('confirmDeleteModal')).hide();
            deleteUserIdToProcess = null;
        }
    });
